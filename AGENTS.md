...
- **No unit reads another unit's state.** There are no Terragrunt `dependency` blocks. A value
  two units share is written once in `env.hcl` or `_envcommon/` and both read it from there —
  so don't add one, and don't add `mock_outputs`. [ADR 015]
...
- **State is per environment, in a PostgreSQL**, with the backend block generated by Terragrunt
  in `root.hcl` from `env.hcl`. **Which PostgreSQL is not this repo's business** — don't assume
  it is the environment's application database, and don't assume it runs in the cluster. The
  connection string is never declared: `env.hcl` names the schema, `PG_CONN_STR` carries the
  rest. [ADR 012]
